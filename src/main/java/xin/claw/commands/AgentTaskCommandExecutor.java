...
public class AgentTaskCommandExecutor extends CommandExecutor {
    ...
    public void onCommand(Command command, String label, String[] args) {
        ...
        if (args.length == 0 || args[0].equalsIgnoreCase("list")) {
            List<Task> tasks = XinClawPlugin.INSTANCE.agentManager.getTaskManager().getTasks();
            if (tasks.isEmpty()) {
            ...
            XinClawPlugin.INSTANCE.agentManager.getTaskManager().addTask(desc);
            logger.info("Added task: {}", desc);
        } else if (args.length >= 2 && args[0].equalsIgnoreCase("rm")) {
            String id = args[1];
            if (XinClawPlugin.INSTANCE.agentManager.getTaskManager().removeTask(id)) {
                logger.info("Removed task: {}", id);
            } else {
                logger.info("Task not found: {}", id);
            }
        } else if (args.length >= 1 && args[0].equalsIgnoreCase("clear")) {
            XinClawPlugin.INSTANCE.agentManager.getTaskManager().clearTasks();
    ...
    public List<String> onTabComplete(Command cmd, String label, String[] args) {
        if (args.length == 1) {
            ...
            if (XinClawPlugin.INSTANCE != null && XinClawPlugin.INSTANCE.agentManager != null) {
                return XinClawPlugin.INSTANCE.agentManager.getTaskManager().getTasks().stream()
                        .map(Task::getId)
