...
public class SocialTools {
    ...
    public String getRecentChat(@P("需要读取的最近行数，最大100") int lines) {
        ...
        if (xin.claw.XinClawPlugin.INSTANCE == null || xin.claw.XinClawPlugin.INSTANCE.chatTracker == null) {
            return "聊天记录器未初始化。";
        }
        java.util.List<String> messages = xin.claw.XinClawPlugin.INSTANCE.chatTracker.getRecentMessages(lines);
        if (messages.isEmpty()) {
            return "最近没有收到任何聊天消息。";
        }
