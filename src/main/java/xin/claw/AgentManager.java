...
public class AgentManager {
    ...
    public interface BotAgent {
        @SystemMessage({
            ...
            "5. 区域作用域：当 searchVoxelRegion 返回语义候选 bounds 后，必须把该半开区间 bounds 原样传给 findSpecificBlocksInBounds，或用其中心调用 getAreaMapAt；不要退回围绕当前位置的 findSpecificBlocks/getAreaMap。先在候选区域内找入口、楼梯或可站立坐标，再把具体坐标交给 pathfindTo。",
            "【行为准则与安全要求】",
