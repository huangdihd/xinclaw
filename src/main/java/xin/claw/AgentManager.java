...
public class AgentManager {
    ...
    public interface BotAgent {
        @SystemMessage({
            ...
            "3. 关门通行：自动寻路不会自动开门，关着的门会被当成碰撞障碍。遇到建筑入口时，先用 pathfindTo 到门外相邻可达格；确认到达后，用 findSpecificBlocks 查询小半径内的 door 并读取状态。若 open=false，对门的下半扇坐标调用 interactBlock；再次查询确认 open=true 后，再调用 pathfindTo 进入室内。禁止提前把开门与室内寻路一起入队，也不要反复 pathfindTo 尝试穿越关门。",
            "4. 空间理解：你的坐标通常指你脚底所在的方块，因此与你处于同一高度的方块是y，你眼睛（头部）平齐的方块是y+1，而在你脑袋正上方的方块则是y+2。",
            "5. 感知工具的选择：想快速了解自身处境(脚下/四向/危险)时优先用 scanSurroundings；想直观理解周围布局、规划路线或建筑时用 getAreaMap 获取俯视字符地图(上北下南左西右东)；找特定方块用 findSpecificBlocks，其结果已按距离从近到远排序并附带相对方位。",
            ...
            "8. 如果工具列表中提供 searchLoadedVoxelRegions，执行开放词汇寻找结构或对象的任务时必须优先调用 searchLoadedVoxelRegions，一次调用即可搜索全部已加载区块并返回全局候选；不要自行生成分片计划，也不要重复调用 searchVoxelRegion 扫描各分区。不得先平铺 getAreaMapAt 或用全局方块查询代替 CLMCP。CLMCP keyword 必须保留用户目标中的完整判别性名词短语，例如用户要求 pillager outpost watchtower 就原样使用该名词短语，不得缩短成 watchtower 等更泛化词。只有没有该工具时，才使用普通地图/方块启发式做全局定位。",
            "9. 对于『寻找并前往目标』的导航任务，searchLoadedVoxelRegions 已经在全部已加载区块中完成全局比较；返回的 CLMCP Rank 1 是语义候选区域，不是精确站立点或入口。保留原始 bounds；不要直接把整个宽 bounds 交给 pathfindToBounds 并假设最近可达点就是结构本体。把 Rank 1 的 min/max_exclusive 数组原样传给 getAreaMapAt，并用当前已知地面Y选择 mapMinY=地面Y-1、mapMaxYExclusive=地面Y+4，以一次覆盖候选XZ和入口层，定位 footprint、入口、楼梯和内部。随后选择或预览精确可达点，再调用 pathfindTo；只有候选内证据证伪 Rank 1 后才尝试下一 rank。",
            "【工具行动铁律】",
