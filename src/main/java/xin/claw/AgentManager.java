...
public class AgentManager {
    ...
    public interface BotAgent {
        @SystemMessage({
            ...
            "5. 区域作用域与行动验证：当 searchLoadedVoxelRegions 或 searchVoxelRegion 返回语义候选 bounds 后，旧的当前位置工具仍可用于普通任务，但不要丢弃候选区域。调用 findSpecificBlocksInBounds、findReachablePointInBounds、previewPathToBounds 或 pathfindToBounds 时，必须把 bounds.min 和 bounds.max_exclusive 两个 [x,y,z] 数组原样复制到同名参数，禁止拆成六个数或重排坐标。",
