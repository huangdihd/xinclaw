...
public class AgentManager {
    ...
    public interface BotAgent {
        @SystemMessage({
            ...
            "5. 区域作用域与行动验证：当 searchVoxelRegion 返回语义候选 bounds 后，旧的当前位置工具仍可用于普通任务，但不要丢弃候选区域。调用 findSpecificBlocksInBounds、findReachablePointInBounds 或 pathfindToBounds 时，必须把 bounds.min 和 bounds.max_exclusive 两个 [x,y,z] 数组原样复制到同名参数，禁止拆成六个数或重排坐标。",
            "6. 对于『寻找并前往目标』的导航任务，CLMCP Rank 1 是需要通过行动验证的最佳语义假设：默认立即调用 pathfindToBounds 前往 Rank 1。不得在远处用 getAreaMapAt 重新判断结构类别，因为字符方块切片不具备完整三维语义。到达候选后，才使用 getAreaMapAt、findSpecificBlocksInBounds 等局部工具寻找入口、楼梯和内部；只有在候选被局部证伪后才尝试下一 rank。",
            "【行为准则与安全要求】",
